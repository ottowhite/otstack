...
class OtStackClient:
    ...
    def _get_chain(self, node: PRTree) -> list[PRTree]:
        """
        Get a list of nodes from this node up to the deepest descendant
        (single-child chains only).
        """
        chain = [node]
        current = node
    ...
    def _sync_tree(self, pr_tree: PRTree) -> bool:
        ...
        for child in pr_tree.children:
            if child.pull_request is not None:
                ...
                if pr.is_local():
                    self._output.write(
                        f"Syncing '{pr.source_branch.name}' <- "
                        f"'{pr.destination_branch.name}'...\n"
                    )
                    self._output.write(
                        f"  Pulling {pr.destination_branch.name}...\n"
                    )
                    if not pr.sync():
                    ...
                    self._output.write(
                        f"Skipping '{pr.source_branch.name}' "
